...
const LittleNotebook = () => {
  ...
  useEffect(async () => {
  axios.get('https://ensemble-tiffany-demo.herokuapp.com/readMsg')
  .then((response) => {
    ...
    for(let i = 0; i < response.data.length; i++){
      ...
      msgtop.innerHTML = response.data.title[i];
      

      let msgBtm = document.createElement("div");
      msgBtm.innerHTML = response.data.msg[i];
...
function searchFunction() {
  ...
  return (
    ...
    <div id="btnContainer">
      ...
      <input placeholder="Search by name" type="text" id="searchFieldName" onKeyUp={searchFunction}/>
      </div>
    <br></br>
      <div id="msgBox"></div>
    </div>
